// index.js const mineflayer = require('mineflayer'); const { pathfinder, Movements, goals: { GoalBlock } } = require('mineflayer-pathfinder'); const Vec3 = require('vec3'); const mcDataLib = require('minecraft-data'); const config = require('./config.json');
...
async function onBotReady() { mcData = mcDataLib(bot.version); defaultMove = new Movements(bot, mcData); defaultMove.canDig = false; bot.pathfinder.setMovements(defaultMove);
...
async function roamLoop() { if (sleeping) return; const center = new Vec3(config.walkCenter.x, config.walkCenter.y, config.walkCenter.z); const radius = 5; const points = [ center.offset(-radius, 0, 0), center.offset(radius, 0, 0), center.offset(0, 0, -radius), center.offset(0, 0, radius) ];
...
try {
  ...
  if (fuelItem) await furnaceWindow.deposit(fuelItem.type, null, fuelItem.count, 1);
  if (smeltItem) await furnaceWindow.deposit(smeltItem.type, null, smeltItem.count, 0);
