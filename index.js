...
bot.once('spawn', () => {
  ...
  defaultMove.allowSprinting = false; // Disable sprinting
  ...
  bot.on('physicTick', () => {
    ...
    if (doorBlock) {
      bot.activateBlock(doorBlock).catch(err => log(`[Door] Failed to open: ${err.message}`));
    }
  });
