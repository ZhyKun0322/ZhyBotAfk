...
const bot = mineflayer.createBot({
  ...
  auth: 'offline' // change to 'mojang' if using Mojang login
});

bot.on('login', () => console.log('✅ Bot logged in'));
bot.on('spawn', onBotReady);
...
function getVec3(configPos) {
  return new Vec3(configPos.x, configPos.y, configPos.z);
}

// --- Behavior functions (same as yours but cleaned and uses config) ---
...
async function roamLoop() {
  if (sleeping) return;
  const center = getVec3(config.walkCenter);
  const points = [
    center.offset(-3, 0, 0),
    center.offset(3, 0, 0),
    center.offset(0, 0, -3),
...
async function farmCrops() {
  const farmMin = getVec3(config.farmMin);
  const farmMax = getVec3(config.farmMax);
...
async function craftBread() {
  ...
  const table = bot.blockAt(getVec3(config.craftingTable));
  if (!table) return;
...
async function storeExcessItems() {
  const chestBlock = bot.blockAt(getVec3(config.chest));
...
async function smeltItemsInFurnace() {
  const furnaceBlock = bot.blockAt(getVec3(config.furnace));
